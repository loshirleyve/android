...
import com.yun9.mobile.framework.task.Task.RunningStatus;
import com.yun9.mobile.framework.task.Task.Status;
...
public class TaskManager {
	...
	public interface IStateChangeListener {
		 ...
		 * 
		 * @param taskManager
		 *            Which task manager's state changed.
		 * @param oldState
		 *            The old state.
		 * @param newState
		 *            The new state.
	 ...
	 */
	// private HandlerThread mThreadWorker = null;
	 ...
	 * 
	 * @param operation
	 *            The task operation contains the task parameter.
	 */
	...
	public void publishProgress(Object progresses) {
		mUIHandler.obtainMessage(MESSAGE_POST_PROGRESS, progresses)
				.sendToTarget();
	}
	...
	private void performStateChange(final State oldState, final State newState) {
		if (null != mListener) {
			mUIHandler.post(new Runnable() {
				@Override
				public void run() {
					mListener.onStateChanged(TaskManager.this, oldState,
							newState);
				}
			});
		}
	 ...
	 * @param newState
	 */
	private void printTaskManagerState(final State oldState,
			final State newState) {
		Log.d("TaskManager", "TaskManager state changed, task manager = "
				+ this.toString());
	}
